<?php 
    ...
    while($data = mysqli_fetch_assoc($res))
    {
      ...
      $table_data .="
        <tr>
          ...
          <td>
            <b>venue:</b> $data[venue_name]
            <br>
            <b>Price:</b> Rs.$data[price]
          </td>
          <td>
            <b>Booking Date:</b> $checkin
          ...
          <td>
            <button type='button' onclick='assign_venue($data[booking_id])' class='btn text-white btn-sm fw-bold custom-bg shadow-none' data-bs-toggle='modal' data-bs-target='#assign-venue'>
              <i class='bi bi-check2-square'></i> Assign venue
            </button>
            <br>
  ...
  if(isset($_POST['assign_venue']))
  {
    $frm_data = filteration($_POST);

    $query = "UPDATE `booking_order` bo INNER JOIN `booking_details` bd
      ON bo.booking_id = bd.booking_id
      SET bo.arrival = ?, bo.rate_review = ?, bd.venue_no = ? 
      WHERE bo.booking_id = ?";

    $values = [1,0,$frm_data['venue_no'],$frm_data['booking_id']];
